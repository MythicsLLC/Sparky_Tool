...
def _vercel_params(team_id: str, extra: dict | None = None) -> dict:
    p = extra or {}
    if team_id:
        p["teamId"] = team_id
    return p
...
async def get_vercel_stats(_: User = Depends(require_admin)):
    cfg = get_settings()
    if not cfg.vercel_token:
        raise HTTPException(503, "VERCEL_TOKEN is not configured — set it in your environment variables")
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        ...
        projects_r = await client.get(
            ...
            params=_vercel_params(team_id, {"limit": "20"}),
        )
        projects_r.raise_for_status()
        projects_data = projects_r.json().get("projects", [])

        # Fetch recent deployments (last 30)
        deploy_r = await client.get(
            f"{base}/v6/deployments",
            headers=_vercel_headers(token),
            params=_vercel_params(team_id, {"limit": "30", "state": "READY,ERROR,CANCELED,BUILDING,QUEUED"}),
        )
        deploy_r.raise_for_status()
        deployments_raw = deploy_r.json().get("deployments", [])
    ...
    deployments = [
        {
            ...
            "meta":        {
                "branch":  d.get("meta", {}).get("githubCommitRef") or d.get("meta", {}).get("gitlabCommitRef") or "",
                "message": d.get("meta", {}).get("githubCommitMessage") or d.get("meta", {}).get("gitlabCommitMessage") or "",
                "author":  d.get("meta", {}).get("githubCommitAuthorName") or "",
            },
            "build_ms": (d.get("ready") - d.get("buildingAt")) if d.get("ready") and d.get("buildingAt") else None,
        }
    ...
    projects = [
        {
            ...
            "node_ver":   p.get("nodeVersion"),
            "updated_at": p.get("updatedAt"),
            "link": {
                "type": p.get("link", {}).get("type", ""),
                "repo": p.get("link", {}).get("repo") or p.get("link", {}).get("projectUrl") or "",
            },
        }
        for p in projects_data
    ]

    # Aggregate quick stats from deployments
    total       = len(deployments)
    prod_ok     = sum(1 for d in deployments if d["state"] == "READY"  and d["target"] == "production")
    prod_err    = sum(1 for d in deployments if d["state"] == "ERROR"  and d["target"] == "production")
    in_progress = sum(1 for d in deployments if d["state"] in ("BUILDING", "QUEUED"))
    build_times = [d["build_ms"] for d in deployments if d["build_ms"] and d["build_ms"] > 0]
    avg_build_s = round(sum(build_times) / len(build_times) / 1000, 1) if build_times else None
